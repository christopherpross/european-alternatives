...
function parseLiveOrigin(value: string | undefined): LiveOriginConfig {
  ...
  if (url.protocol !== 'https:') {
    return {
      error:
        'EUROALT_LIVE_BASE_URL must use https:// so deployment verification matches the public nosniff origin.',
    }
  }
...
async function fetchLiveHeaders(
  ...
  if (!liveOriginConfig.origin) {
    throw new Error(
      'Live X-Content-Type-Options smoke test requested without EUROALT_LIVE_BASE_URL.',
    )
  }
...
async function waitForLiveHeaderCheck(
  ...
  while (true) {
    ...
    try {
      ...
      const actualXContentTypeOptionsValue = response.headers.get(
        'x-content-type-options',
      )
      const mismatches: string[] = []

      if (response.status === check.expectedStatus) {
        if (actualXContentTypeOptionsValue === xContentTypeOptionsValue) {
          return null
        }
      } else {
        mismatches.push(
      ...
      if (actualXContentTypeOptionsValue !== xContentTypeOptionsValue) {
        mismatches.push(
          `X-Content-Type-Options ${JSON.stringify(actualXContentTypeOptionsValue)} instead of ${JSON.stringify(xContentTypeOptionsValue)}`,
        )
      }
...
describe('live X-Content-Type-Options deployment smoke test configuration', () => {
...
describeIfLive('live X-Content-Type-Options deployment smoke test', () => {
  const checks: LiveHeaderCheck[] = [
    { label: 'the root document', path: '/', expectedStatus: 200 },
    {
      label: 'the stable app shell JavaScript asset',
      path: '/app-shell.js',
      expectedStatus: 200,
    },
    { label: 'a static SVG asset', path: '/favicon.svg', expectedStatus: 200 },
    {
  ...
  it(
    'returns a real nosniff header for the public root, JS/static assets, API, and 404 coverage set',
    async () => {
